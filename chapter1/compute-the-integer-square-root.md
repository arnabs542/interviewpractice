...
For example, if $$\small k = 21$$, we initialize the interval to $\[0,21\]

The midpoint small m = \lfloor\(0 + 21\)/2\rfloor = 10$$; since $$10^{2} &gt; 21$$, we update the interval to $$\small \[0, 9\]$$. Now $$\small m = \lfloor\(0 + 9  
\)/2\rfloor = 4$$; since
